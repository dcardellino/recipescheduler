...
- [ ] **TASK-001** — Next.js 15 Projekt initialisieren mit TypeScript, Tailwind, App Router
...
- [ ] **TASK-002** — shadcn/ui initialisieren und Basis-Komponenten installieren
...
- [ ] **TASK-003** — Custom Tailwind-Theme mit Design-Tokens einrichten
  Files: `tailwind.config.ts`, `src/app/globals.css`
  Notes: Farben aus PRD § 9. Fonts via `next/font/google` (Fraunces für heading, Inter für body).

- [ ] **TASK-004** — Postgres lokal via Docker Compose starten
...
- [ ] **TASK-005** — Drizzle ORM installieren und konfigurieren
...
- [ ] **TASK-006** — Users, Sessions, VerificationTokens Schema anlegen (better-auth Felder)
...
- [ ] **TASK-007** — Households + HouseholdMembers Schema hinzufügen
  Files: `src/db/schema.ts`
  Notes: Siehe PRD § 3. `householdMembers` mit enum role.

- [ ] **TASK-008** — Initiale Migration erstellen und ausführen
  Files: `src/db/migrations/*`
  Notes: `npx drizzle-kit generate` dann `npx drizzle-kit migrate`.

- [ ] **TASK-009** — better-auth Setup + Magic Link Plugin
  Files: `src/lib/auth.ts`, `src/app/api/auth/[...all]/route.ts`
  Notes: Siehe PRD § 10. `BETTER_AUTH_SECRET` in .env.

- [ ] **TASK-010** — Resend Account anlegen + API Key + sendMagicLinkEmail Helper
...
- [ ] **TASK-011** — Login-Seite implementieren (E-Mail → Magic Link)
...
- [ ] **TASK-012** — Middleware für Protected Routes
  Files: `src/middleware.ts`
  Notes: Siehe PRD § 10. Redirect zu `/login` wenn keine Session.

- [ ] **TASK-013** — Post-Login Household-Auto-Creation Hook
...
- [ ] **TASK-014** — Placeholder-Layout mit Top-Nav (Desktop) + Bottom-Nav (Mobile)
...
- [ ] **TASK-018** — Test-Seite `/dev/upload-test` zum Verifizieren des Flows
...
- [ ] **TASK-019** — Dockerfile für Next.js App
...
- [ ] **TASK-028** — Rezept-Erstellen-Seite (manuell)
...
- [ ] **TASK-039** — URL-Fetch + JSON-LD-Parser implementieren
...
- [ ] **TASK-040** — Einheiten-Normalisierung für importierte Zutaten
...
- [ ] **TASK-041** — Kategorie-Klassifikation für importierte Zutaten
...
- [ ] **TASK-042** — API Route `/api/recipes/import`
...
- [ ] **TASK-043** — "Rezept importieren" UI: URL-Input → Preview-Form
...
- [ ] **TASK-044** — Fallback bei Import-Fehler: Manual-Form mit vorausgefülltem Titel
...
- [ ] **TASK-045** — Foto-Import aus Meta-Tags (`og:image` / JSON-LD `image`)
...
- [ ] **TASK-046** — Test mit 5 realen Kochseiten (Chefkoch, NYT, Kitchen Stories, BBC Good Food, einfachbacken.de)
...
- [ ] **TASK-047** — MealPlanEntries, ShoppingLists, ShoppingListItems Schema
  Files: `src/db/schema.ts`
  Notes: Siehe PRD § 3. Migrations generieren + ausführen.

- [ ] **TASK-048** — Server Actions: addMealPlanEntry, removeMealPlanEntry, updateServings
...
- [ ] **TASK-049** — Wochenplan-Seite Desktop (7-Spalten-Grid)
...
- [ ] **TASK-050** — Wochenplan-Seite Mobile (Liste mit 7 Sections)
  Files: `src/components/week/week-list.tsx`
  Notes: Responsive: unter 768px die Liste, darüber das Grid.

- [ ] **TASK-051** — Rezept-Picker-Sheet (Mobile + Desktop)
  Files: `src/components/week/recipe-picker.tsx`
  Notes: Sheet mit Suche + Tag-Filter (reuse von Phase 1). Klick → zu Tag hinzufügen.

- [ ] **TASK-052** — Portionen-Anpassung pro Entry (Inline-Input)
  Files: `src/components/week/day-entry.tsx`
  Notes: Optimistic UI. Bei Blur → Server Action.

- [ ] **TASK-053** — Entry löschen (Swipe mobile, Click desktop)
  Files: `src/components/week/day-entry.tsx`
  Notes: `react-swipeable` oder CSS scroll-snap. Confirm bei Mobile-Swipe.

- [ ] **TASK-054** — Woche-Navigation (vor/zurück Buttons + aktuelle Woche anzeigen)
  Files: `src/components/week/week-header.tsx`
  Notes: ISO-Wochennummer anzeigen ("KW 17").

- [ ] **TASK-055** — Shopping-List-Aggregationslogik
  Files: `src/lib/shopping-list.ts`, `src/lib/shopping-list.test.ts`
  Notes: Siehe PRD § 4 Pseudocode. Unit-Converter für g↔kg, ml↔l, TL↔EL. Umfassende Tests!

- [ ] **TASK-056** — Server Action: generateShoppingList
  Files: `src/actions/shopping.ts`
  Notes: Idempotent: falls Liste für Woche existiert, prüfen ob überschreiben oder mergen (MVP: overwrite mit Confirm).

- [ ] **TASK-057** — "Einkaufsliste generieren"-Button im Wochenplan
...
- [ ] **TASK-059** — Item abhaken mit Optimistic UI
  Files: `src/components/shopping/item-row.tsx`, `src/actions/shopping.ts`
  Notes: Client-State mit useOptimistic. Fehler-Handling: revert + Toast.

- [ ] **TASK-060** — Custom-Item hinzufügen (Name + optional Menge/Einheit/Kategorie)
  Files: `src/components/shopping/add-custom-item.tsx`
  Notes: Inline-Form am Ende der Liste.

- [ ] **TASK-061** — Progress-Anzeige "X von Y erledigt"
  Files: `src/components/shopping/progress-bar.tsx`
  Notes: Sticky Top-Header.

- [ ] **TASK-062** — Historie: Ältere Einkaufslisten zugreifbar
  Files: `src/app/(app)/shopping/page.tsx` (Tabs: "Aktuell" / "Verlauf")
  Notes: List der letzten 8 Wochen.

- [ ] **TASK-063** — E2E-Test für den Magic Moment (Playwright)
  Files: `e2e/magic-moment.spec.ts`
  Notes: Login → 5 Rezepte in Woche planen → Einkaufsliste generieren → alle Kategorien sichtbar → ein Item abhaken → Reload → State persistent.
...
## Phase 4 — PWA + Offline + Polish

**Goal:** Installierbar auf Handy, Einkaufsliste offline nutzbar, UX-Schliff.
**Demo:** App auf iPhone installiert, im Supermarkt (Flugmodus) Einkaufsliste abgehakt, Reconnect → Sync.
**Reference-Sections:** PRD § 6 (FR-020, FR-021, FR-024), § 7 (Performance, Accessibility).

- [ ] **TASK-064** — Serwist installieren + konfigurieren
  Files: `next.config.mjs`, `src/app/sw.ts`, `public/manifest.json`
  Notes: `@serwist/next`. Precache Shell-Routen, Runtime-Cache für API.

- [ ] **TASK-065** — PWA Manifest + Icons
  Files: `public/manifest.json`, `public/icons/icon-192.png`, `public/icons/icon-512.png`, `public/icons/maskable-512.png`
  Notes: Icons im Terracotta/Cream Design. Optional: Tool wie PWA Builder nutzen.

- [ ] **TASK-066** — Offline-Strategie für Shopping-Seite
  Files: `src/app/sw.ts`, `src/components/shopping/offline-sync.tsx`
  Notes: Cache-First für /shopping und assets. IndexedDB-Queue für Toggle-Actions. Background Sync API optional.

- [ ] **TASK-067** — Offline-Indikator
  Files: `src/components/layout/offline-banner.tsx`
  Notes: `navigator.onLine` + `online`/`offline` Events. Banner sichtbar wenn offline.

- [ ] **TASK-068** — Lighthouse-Audit durchführen und Issues fixen
  Files: diverse
  Notes: Ziel: PWA-Score ≥90, Accessibility ≥95, Performance ≥80. Fix typische Issues (Image-Dimensionen, Contrast, Labels).

- [ ] **TASK-069** — Tastatur-Navigation testen und fixen
  Files: diverse
  Notes: Tab-Order, Focus-Visible, Esc schließt Dialoge, Enter submitted Forms.

- [ ] **TASK-070** — Loading-Skeletons für alle Pages
  Files: `src/app/(app)/**/loading.tsx`
  Notes: Next.js `loading.tsx` Convention.

- [ ] **TASK-071** — Error-Boundaries + freundliche Error-Pages
  Files: `src/app/(app)/**/error.tsx`, `src/app/global-error.tsx`
  Notes: Reset-Button, Fehler-ID für Debugging.

- [ ] **TASK-072** — Toast-Feedback für alle Actions (sonner)
  Files: diverse Server Actions + Components
  Notes: Erfolg: "Rezept gespeichert." / Fehler: konkrete Meldung.

- [ ] **TASK-073** — Deutsche Datumsformatierung überall
  Files: `src/lib/date.ts`
  Notes: `date-fns` mit `de` locale. Format "Mo, 17. Apr".

**Phase 4 Summary Prompt für Claude Code:**
> "Phase 4 umsetzen: Aus der Web-App wird eine installierbare PWA mit Offline-Support für die Einkaufsliste. Lighthouse-Scores verbessern, Accessibility-Issues fixen, UI-Polish überall."
...
- [ ] **TASK-074** — Settings-Page: Household-Section
...
- [ ] **TASK-075** — Invite-Flow: Mitglied einladen
...
- [ ] **TASK-076** — Invite-Link-Handler: Neuer User wird Household-Member nach Login
...
- [ ] **TASK-079** — Account-Section: Abmelden-Button
  Files: `src/components/settings/account-section.tsx`
  Notes: Sign-out Action → redirect zu /login.

- [ ] **TASK-080** — Authorization-Layer: alle Reads/Writes prüfen auf Household-Membership
...
- [ ] **TASK-081** — E2E-Test: Zwei-User-Flow
...
- [ ] **TASK-090** — Provider-Wahl: Claude API (Anthropic) oder OpenAI
...
- [ ] **TASK-096** — Wochenplan duplizieren / als Template speichern
- [ ] **TASK-097** — Rezept-Historie "zuletzt gekocht am"
- [ ] **TASK-098** — Rezept als PDF exportieren
- [ ] **TASK-099** — Dark Mode (System-adaptive)
- [ ] **TASK-100** — Command Palette (Cmd+K)
- [ ] **TASK-101** — "Gäste heute?" Flag pro Entry → Portionen ×2
